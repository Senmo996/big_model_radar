...
| `src/signals.ts` | Deterministic cross-source evidence normalization, clustering, and signal scoring |
| `src/backfill-signals.ts` | Builds a signal bundle from an existing historical digest date |
...
- Empty successful LLM responses are retried up to 3 times (1 s / 2 s / 4 s). If all attempts are empty, the call fails with safe diagnostics (`finish_reason`, token usage, and whether `reasoning_content` was present).
...
- `src/generate-manifest.ts` excludes generated reports whose body is empty, so broken output is not linked from the Web UI, RSS feed, or notifications.
- The same manifest step writes `search-index.json`; the Web UI and MCP server use it instead of bulk-fetching all historical Markdown files.
- Daily runs write `digests/YYYY-MM-DD/signals.json`. Signal facts and metrics come directly from source data; keep the clustering/scoring path independent of LLM output.
